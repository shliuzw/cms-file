(function( $ ){
    ...
    $(function() {
        ...
        uploader = WebUploader.create({
            pick: { // 指定选择文件的按钮容器
                id: '#filePicker',
                label: '点击选择图片' // 指定按钮文字。不指定时优先从指定的容器中看是否自带文字。
            },
            ...
            server: jypath +'/backstage/tool/webuploader/uploadFilePic?fileId=' +  fileId,
        ...
        uploader.on( 'uploadAccept', function( file, response ) {
            ...
            if ( response.res==1 ) {
                ...
                $.gritter.add({
                    title: '文件' + file.file.name + "上传成功",
                    text: '文件' + file.file.name + "上传成功",
                    sticky: false,
                ...
                console.log(response.resMsg);
                $.gritter.add({
                    title: "文件" + file.file.name + "上传失败",
                    text: "失败原因" + response.resMsg,
                    sticky: false,
                    before_open: function(){
                        if($('.gritter-item-wrapper').length >= 3)
                        {
