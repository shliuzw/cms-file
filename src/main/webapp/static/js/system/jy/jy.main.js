...
JY={
	...
	Dict:{//ids 对应id值(多个逗号分隔).keys 对应key值(多个逗号分隔).type(可选)1.请选择，2.自定义数组。默认不填.dfstr (可选)自定义数组
		setSelect:function(ids,keys,type,dfstr,value){
			$.ajax({type:'POST',url:jypath+'/backstage/dataDict/getDictSelect',
				data:{ids:ids,keys:keys},dataType:'json',
		     		...
		     		if(type==1){
		     			for(var i=0;i<idss.length;i++){
		     				...
		     				$.each(map[idss[i]].items,function(n,v) {
		     					if(v.value == value){
		     						opts+="<option selected value='"+v.value+"'>"+v.name+"</option>";
		     					}else{
		     			...
		     			for(var i=0;i<idss.length;i++){
		     				...
		     				$.each(map[idss[i]].items,function(n,v) {opts+="<option value='"+v.value+"'>"+v.name+"</option>";	});
		     				$("#"+idss[i]+" select").append(opts);
		     			};
		     		}else{
		     			for(var i=0;i<idss.length;i++){
	...
	Tags:{//设置按钮用的方法,id 这行的id,pBtn 按钮组
		...
		cleanForm:function(formId){
			$("#"+formId+" input[type$='text']").val(""); //表单中input[type$='text'] 文本输入框初始化为空
			$("#"+formId+" textarea").val(""); // 表单中的textarea元素的值初始化为空
		},
	...
	Model:{//loading框
		...
		check:function(id,title,fn){
			$("#"+id).removeClass('hide').dialog({resizable:false,modal:true,width:500,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"查看")+"</h4></div>",title_html:true,buttons:[{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(fn) == 'function'){fn.call(this);}}}]});
		},
		// 添加，修改页面
		edit:function(id,title,savefn,cancelfn){
			$("#"+id).removeClass('hide').dialog({
				...
				buttons:[{
					...
					click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}
					},{
		...
		// 跟踪明细页面
        table:function(id,title,savefn,cancelfn){
        	$("#"+id).removeClass('hide').dialog({
        		resizable:true,
        		modal:false,
        		width:900,
		...
		editBig:function(id,title,fileId){
        	$("#"+id).removeClass('hide').dialog({
        		resizable:true,
        		modal:true,
        		width:800,
        	...
        	$("#uploader").attr("fileId", fileId);
        	uploader.options.server = jypath +'/backstage/tool/webuploader/uploadFilePic?fileId=' +  fileId;
        	$("#filePicker").children("div[id^='rt_rt_']").css({"width":"168px","height":"44px","top":"20px", "left":"260px" })
        },
         //图片
		imgPreview:function(id, title, fileId){
			$("#"+id).removeClass('hide').dialog({
			resizable:true,
			modal:false,
			width:800,
