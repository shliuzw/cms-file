...
//	JY.Dict.setSelect("selectisValid","isValid",2,"全部");
	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auForm",jypath +'/backstage/file/add',null,function(data){
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){	
				JY.Ajax.doRequest(null,jypath +'/backstage/file/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});		
		}
    ...
    $('#copyBatchBtn').on('click', function(e) {
    	//通知浏览器不要执行与事件关联的默认动作
        e.preventDefault();
        JY.Ajax.doRequest(null,jypath +'/backstage/file/copyBatchInit',null,function(data){
            setFormBackup(data); // 修改表单赋值
            JY.Model.edit("auDivBackup","备份文件",function(){
            	 if(JY.Validate.form("auFormBackup")){
            		 var that =$(this);
            		 JY.Ajax.doRequest("auFormBackup",jypath +'/backstage/file/copyBatch',null,function(data){
            		    that.dialog("close");// 关闭窗口
            		 });
            	 }
            });
...
function loadOrgTree(){
	JY.Ajax.doRequest(null,jypath +'/backstage/file/orgTree',null,function(data){
		$.fn.zTree.init(
			$("#orgTree"),
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",jypath +'/backstage/file/findByPage',null,function(data){
...
function check(accountId){
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/file/orgTree',null,function(data){
		...
		JY.Ajax.doRequest(null,jypath +'/backstage/file/find',{accountId:accountId},function(data){
		    setForm(data);
...
function del(fileId){
	JY.Model.confirm("确认删除吗？",function(){	
		JY.Ajax.doRequest(null,jypath +'/backstage/file/del',{fileId:fileId},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
...
function loadFileImages(fileId, pageNow){
	JY.Ajax.doRequest(null,jypath +'/backstage/fileImage/findByPage',{"fileId":fileId, "pageNum" : pageNow},function(data){
		if(data && data.res == 1){
			if(data.obj.list.totalRecord>0){
				...
				$.each(data.obj.list.results, function(index, fiItem){
					...
					imgList += "<a href='javascript:void(0);' imgUrl='"+jypath  + fiItem.barcode+ "' data-rel='colorbox' img-index='"+index+"' class='cboxElement' onclick='imgOverView(\" "+ jypath  + fiItem.barcode+"\","+index+","+viewCount+");'>"+
									"<img width='150px' height='150px' alt='150x150' src='"+jypath + fiItem.barcode+"'> </a>"+
...
function fileImageDel(imageId){
	JY.Ajax.doRequest(null,jypath +'/backstage/fileImage/delFileImage',{"imageId":imageId},function(data){
		if(data.res == 1){
			console.log("成功删除该图片");
...
function edit(fileId){
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/file/orgTree',null,function(data){
		$.fn.zTree.init(
			...
			data.obj);
		JY.Ajax.doRequest(null,jypath +'/backstage/file/find',{fileId:fileId},function(data){
		    setForm(data); // 修改表单赋值
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					var that =$(this);
					JY.Ajax.doRequest("auForm",jypath +'/backstage/file/update',null,function(data){
					    that.dialog("close");
					    JY.Model.info(data.resMsg,function(){search();});
					});
				}
