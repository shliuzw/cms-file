	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auForm",jypath +'/backstage/scangun/add',null,function(data){
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){	
				JY.Ajax.doRequest(null,jypath +'/backstage/scangun/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});		
		}
...
function loadOrgTree(){
	JY.Ajax.doRequest(null,jypath +'/backstage/scangun/orgTree',null,function(data){
		$.fn.zTree.init(
			$("#orgTree"),
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",jypath +'/backstage/scangun/findByPage',null,function(data){
...
function check(gunId){
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/scangun/orgTree',null,function(data){
		...
		JY.Ajax.doRequest(null,jypath +'/backstage/scangun/find',{gunId:gunId},function(data){
		    setForm(data);
...
function del(gunId){
	JY.Model.confirm("确认删除吗？",function(){
		JY.Ajax.doRequest(null,jypath +'/backstage/scangun/del',{gunId:gunId},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/scangun/orgTree',null,function(data){
		...
		JY.Ajax.doRequest(null,jypath +'/backstage/scangun/find',{gunId:gunId},function(data){
		    ...
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					var that =$(this);
					JY.Ajax.doRequest("auForm",jypath +'/backstage/scangun/update',null,function(data){
					    that.dialog("close");
					    JY.Model.info(data.resMsg,function(){search();});
					});
				}
