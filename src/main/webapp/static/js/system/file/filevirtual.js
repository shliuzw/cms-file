	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auForm",jypath +'/backstage/filevirtual/add',null,function(data){
	...
	$('#delBatchBtn').on('click', function(e) {
		...
		if(chks.length==0) {
			...
			JY.Model.confirm("确认要删除选中的数据吗?",function(){	
				JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/delBatch',{chks:chks.toString()},function(data){
					JY.Model.info(data.resMsg,function(){search();});
				});
			});		
		}
...
function loadOrgTree(){
	JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/orgTree',null,function(data){
		$.fn.zTree.init(
			$("#orgTree"),
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",jypath +'/backstage/filevirtual/findByPage',null,function(data){
...
function check(fileId){
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/orgTree',null,function(data){
		...
		JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/find',{fileId:fileId},function(data){
		    setForm(data);
...
function del(fileId){
	JY.Model.confirm("确认删除吗？",function(){	
		JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/del',{fileId:fileId},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/orgTree',null,function(data){
		...
		JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/find',{fileId:fileId},function(data){
		    ...
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					var that =$(this);
					JY.Ajax.doRequest("auForm",jypath +'/backstage/filevirtual/update',null,function(data){
					    that.dialog("close");
					    JY.Model.info(data.resMsg,function(){search();});
					});
				}
...
function getbaseTrackerList(init,fileId){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseFormTracker",jypath +'/backstage/filevirtualTracker/findByPage',{fileId:fileId},function(data){
...
function createBarcode(fileId){
	JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/createBarcode',{fileId:fileId},function(data){
	    JY.Model.info("条形码成功生成!");
	});
}
// 生成封面
function createFace(fileId){
	JY.Ajax.doRequest(null,jypath +'/backstage/filevirtual/createFace',{fileId:fileId},
		function(data){
