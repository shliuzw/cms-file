	...
	$('#editOrgBtn').on('click', function(e) {
		...
		JY.Ajax.doRequest(null,jypath+'/backstage/org/role/findOrg',{id:orgId},function(data){
			if(data.res==1){
				// 页面属性赋值数据
				setOrgForm(data);    
				JY.Model.edit("auOrgDiv","修改组织",function(){
					 if(JY.Validate.form("auOrgForm")){
						 var that =$(this);
	...
	JY.Ajax.doRequest(null,jypath +'/backstage/org/role/getOrgTree',null,function(data){
		...
		$.fn.zTree.init(
			...
			{	view:{selectedMulti:false,fontCss:{color:"#393939"}},
				...
				callback:{onClick: clickOrg} // 指定组织的单击事件clickOrg
			},
			data.obj
		);
		var treeObj = $.fn.zTree.getZTreeObj("orgTree");
...
function clickOrg(event,treeId,treeNode) {
	...
	getbaseList(); // 获取当前组织的角色列表
}
// 获取当前组织的角色列表，并组织显示的HTML表格
function getbaseList(init){
	...
	JY.Ajax.doRequest("baseForm",jypath +'/backstage/org/role/findByPage',null,function(data){
		 $("#baseTable tbody").empty();
    		 var obj=data.obj;
        	 var list=obj.list; // Page<Role>
        	 var results=list.results; // 角色list
         	 var pageNum=list.pageNum,pageSize=list.pageSize,totalRecord=list.totalRecord;
         	 var permitBtn=obj.permitBtn; // 角色操作权限
        	 var html="";
...
function setOrgForm(data){
	var l=data.obj;
	$("#auOrgForm input[name$='id']").val(JY.Object.notEmpty(l.id)); // 组织id
	$("#auOrgForm input[name$='isValid']").val(JY.Object.notEmpty(l.isValid)) // 组织状态
	$("#auOrgForm input[name$='name']").val(JY.Object.notEmpty(l.name)); // 组织名称
	$("#auOrgForm textarea[name$='description']").val(JY.Object.notEmpty(l.description));//组织描述
	$("#preOrg").val(JY.Object.notEmpty(l.pName)); // 父组织名称
	$("#auForm input[name$='pId']").val(JY.Object.notEmpty(l.pId));//父组织ID
};
